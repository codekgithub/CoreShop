<template>
	...
	<view
		...
		v-bind:class="coreshopdata.parameters.style">
		<u-toast ref="uToast" />
		<u-search :placeholder="coreshopdata.parameters.keywords" v-model="keyword" shape="square" :show-action="true"
			action-text="搜索" @custom="goSearch" @search="goSearch"></u-search>
	</view>
</template>
<script>
	export default {
		...
		created() {
			...
			this.$nextTick(() => {
				...
				if (this.$refs.searchBar) this.searchTop = this.$refs.searchBar.$el.offsetTop;
			})
			// #endif
			this.searchStyle()
		},
